...
<p>If you don\'t know how to set up a database you should <strong>contact your host</strong>.</p>', $db, $this->dbuser), 'db_select_fail');
	...
	public function prepare( $query, ...$args ) {
		...
		if ( $args_count !== $placeholder_count ) {
			if ( 1 === $placeholder_count && $passed_as_array ) {
				...
				trigger_error(
                    sprintf(
					    'The query only expected one placeholder, but an array of multiple placeholders was sent. Query: %s',
                        $query
                    ),
					E_USER_WARNING
				);
				...
				trigger_error(
					sprintf(
						'The query does not contain the correct number of placeholders %s for the number of arguments passed %s. Query: %s',
						$placeholder_count,
						$args_count,
                        $query
					),
	 ...
	 *                Call idb::prepare() or idb::_real_escape() next.
	 */
	...
	public function print_error( $str = '' ) {
		...
		printf(
			'<div id="error"><p class="idberror"><strong>Database error:</strong> [%s]<br /><code>%s</code></p></div>',
			$str,
			$query
		);
	}
	 ...
	 * idb::hide_errors() should be used instead for hiding errors.
     *
     * @since 0.71
     *
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( DEBUG ) {
			...
			// phpcs:ignore PHP.NoSilencedErrors.Discouraged
			@mysqli_real_connect( $this->dbh, $host, $this->dbuser, $this->dbpassword, null, $port, $socket, $client_flags );
		}
...
" ), htmlspecialchars( $this->dbhost, ENT_QUOTES ) ), 'db_connect_fail' );
	 ...
	 *                                specified in idb::$field_types. Default null.
	 ...
	 *
	 * This is a helper method for idb's CRUD methods, which take field/value pairs
	...
	/**
	 * Adds field charsets to field/value/format arrays generated by idb::process_field_formats().
	 *
	 * @since 4.2.0
	 * @access protected
	 *
	 * @param array $data {
	 *     Array of values and formats keyed by their field names,
	 *     as it comes from the idb::process_field_formats() method.
	 *
	 *     @type array ...$0 {
	 ...
	 *     as it comes from the idb::process_field_charsets() method.
	 *
	 *     @type array ...$0 {
	 *         Value, format, and charset for this field.
	 *
	...
	public function get_col_charset( $table, $column ) {
		...
		if ( empty( $this->table_charset[ $tablekey ] ) ) {
			...
			if ( $table_charset == 'error' ) {
			// if ( might_keep_is_wp_error( $table_charset ) ) {
	 ...
	 *                        such as 'field' are retained in each value array.
     *                        If we cannot remove invalid characters
	 */
	protected function strip_invalid_text( $data ) {
		...
		if ( $db_check_string ) {
			...
			if ( ! $row ) {
                return 'error';
				// return trigger_error("'strip_invalid_text': Could not strip invalid text", E_USER_ERROR);
			}
	...
	protected function strip_invalid_text_from_query( $query ) {
		...
		if ( $data == 'error' ) {
		// if ( might_keep_2_is_wp_error( $data ) ) {
			return $data;
		}
	 ...
	 *
	 * Will not die if odb::$show_errors is false.
	 *
	 * @since 1.5.0
	 *
	...
	/**
	 * Determines whether the database or iDB supports a particular feature.
	 *
	 * Capability sniffs for the database server and current version of iDB.
	 *
	 * Database sniffs are based on the version of MySQL the site is using.
	 *
	 * iDB sniffs are added as new features are introduced to allow theme and plugin
	 * developers to determine feature support. This is to account for drop-ins which may
	 * introduce feature support at a different time.
	 *
	 * @since 2.7.0
	...
	public function has_cap( $db_cap ) {
		...
		switch ( strtolower( $db_cap ) ) {
			...
			case 'identifier_placeholders': // @since 6.2.0
				/*
				 * As of WordPress 6.2, idb::prepare() supports identifiers via '%i',
    ...
    /**
	 * Retrieves a comma-separated list of the names of the functions that called idb.
     *
